...
object BigramAnalysis {
  def main(arg: Array[String]) = {
    ...
    val inputFilePath = if(argsValid) arg(1) else "hdfs:///tmp/bible-shakes.nopunc.gz"

    val outputFilePath = if(argsValid) arg(1) else "hdfs:///tmp/bigramWordOccuranceCount.txt"
    ...
    startWordBGbGCountStartWordBgsCount.saveAsTextFile(outputFilePath)
